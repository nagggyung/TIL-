...
8. At the end of each epoch during the training process, the loss will be calculated using the network's output predictions and the true labels for the respective input. 
...
Given that the objective of SGD is to minimized the loss, we want to see our loss decrease as we run more epochs.
